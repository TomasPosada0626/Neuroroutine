...
-- Baseline table/RLS bootstrap so this migrations/ directory is replayable from an empty
-- database on its own (needed by `supabase start`/`supabase db reset` and pgTAP CI).
-- Previously, migrations 0001+ were incremental ALTERs that silently assumed routines/
-- routine_tasks/profiles already existed from a pre-migrations-era manual setup via
-- schema.sql — a fresh `supabase start` failed on migration 0001 for exactly this reason.
-- This file is schema.sql's content verbatim (fully idempotent: if-not-exists/or-replace/
-- drop-then-create throughout), so re-running it against the already-provisioned real
