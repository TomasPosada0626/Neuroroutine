...
-- Supports get_action_latency_p95() below: filtering by event_name + created_at had no index
-- before (app_events_user_created_idx above is keyed by user_id first). Migration 0019.
create index if not exists app_events_name_created_idx
  on public.app_events (event_name, created_at desc);

-- Read-only aggregate for the runtime UX budget in docs/architecture/performance-budget.md
-- (p95 routine-create <=800ms, p95 task-complete <=600ms). No PII, no per-user rows - same
-- sensitivity class as get_nr_schema_status() below, which is also anon-grantable. Migration 0019.
