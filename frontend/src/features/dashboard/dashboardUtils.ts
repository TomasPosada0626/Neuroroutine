...
export function computeDayActivitySet(events: RoutineTaskEvent[], filter?: { routineId?: string | null }) {
...
export function computeWeekCounts(
  ...
  const deltaPct = prevWeekCompleted === 0 ? (thisWeekCompleted > 0 ? 100 : 0) : ((thisWeekCompleted - prevWeekCompleted) / prevWeekCompleted) * 100
